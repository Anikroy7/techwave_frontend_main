                ...
                <tbody className=" text-sm font-light">
                    {orders.map((order) => (
                        <tr key={order.transactionId} className="border-b border-gray-300 ">
                            ...
                            <td className={`py-3 px-6 font-semibold ${order.paymentStatus === 'Paid' ? 'text-green-500' : order.paymentStatus === 'Failed' ? 'text-red-500' : 'text-yellow-500'}`}>
                                {order.paymentStatus}
                            </td>
                            <td className="py-3 px-6">{order.transactionId}</td>
                            <td className="py-3 px-6">${order.totalPrice.toFixed(2)}</td>
                            <td className="py-3 px-6">{new Date(order.startDate).toLocaleDateString()}</td>
                            <td className="py-3 px-6">{new Date(order.endDate).toLocaleDateString()}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
